...
class clinicalPharmacologyProp(SchemaProperty):

    """
    SchemaField for clinicalPharmacology
    Usage: Include in SchemaObject SchemaFields as your_django_field = clinicalPharmacologyProp()  
    schema.org description:Description of the absorption and elimination of drugs, including their concentration (pharmacokinetics, pK) and biological effects (pharmacodynamics, pD). Supersedes clincalPharmacology.

    prop_schema returns just the property without url#
    format_as is used by app templatetags based upon schema.org datatype
    """

    _prop_schema = 'clinicalPharmacology'
    _expected_schema = None
    _enum = False
    _format_as = "TextField"
