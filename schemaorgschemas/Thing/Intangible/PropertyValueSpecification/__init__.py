...
class stepValueProp(SchemaProperty):
    ...
    schema.org description:The stepValue attribute indicates the granularity that is expected (and required) of the value in a PropertyValueSpecification.
...
class valueNameProp(SchemaProperty):

    """
    SchemaField for valueName
    Usage: Include in SchemaObject SchemaFields as your_django_field = valueNameProp()  
    schema.org description:Indicates the name of the PropertyValueSpecification to be used in URL templates and form encoding in a manner analogous to HTML&#39;s input@name.

    prop_schema returns just the property without url#
    format_as is used by app templatetags based upon schema.org datatype
    """

    _prop_schema = 'valueName'
    _expected_schema = None
    _enum = False
    _format_as = "TextField"
