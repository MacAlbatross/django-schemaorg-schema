...
class SchemaTemplateTagTests(TestCase):
    ...
    def test_set_all(self):
        out = Template("{%load schemadisplay %}"
                       ...
                       """<table>{% for book in object.book_set.all %}<tr><td>{{book.title}}</td><td>{{book.publication_date}}</td></tr>{% endfor %}</table>"""
                       """{% endschemaobject %}"""
    ...
    def test_nested_html(self):
        out = Template("{%load schemadisplay %}"
                       """{% schemaobject object section "class='author'" "author-id-" %}<span>{{object.first_name}}</span>&nbsp;<span>{{object.last_name}}</span>"""
                       """<table>{% for book in object.book_set.all %}<tr><td><a href="{{book.get_absolute_url}}">{{book.title}}</a></td><td>{{book.publication_date}}</td></tr>{% endfor %}</table>"""
                       """{% endschemaobject %}"""
                       ).render(Context({'object': Author.objects.get(id=1)}))
